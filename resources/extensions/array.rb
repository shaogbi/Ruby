...
###### group_by ######
# convert an array to formatted hash, usage:
raw = [
...
h = raw.group_by(slices, len, {:mode=>"total",:format=>"delimiter"})
...
class Array
  ##############################################################
  # following are helper methods of "group_by"
  # 87.621 -> 87.62%
  def get_percentage_str(num)
    return format("%0.2f", num).to_s + "%"
  end
  private :get_percentage_str
  ...
  private :get_total_mode
  ##############################################################

  def group_by(slices, ts_len, opts = {})
    len = slices.length
    res = get_initial_hash(len)
    str = (0 ... len).reduce("") { |curr, val| curr += "[dp[slices[#{val}]]]" }
    self.each do |dp|
